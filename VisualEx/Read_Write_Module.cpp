...
//
#define _CRT_SECURE_NO_WARNINGS  // HK: just for visual studio GCC compiler to not classify errors as warnings. take it out for you
...
//HK: splitting up filemod

vector<vector<string>> RWModule::convert2D(vector<string> ParsedData)
{
vector<vector<string>> record(12, vector<string>(5));
    // Read data into array
...
vector<vector<string>> RWModule::fileMod(vector<vector<string>> record){
...
vector<vector<int>> RWModule::StringtoInt(vector<vector<string>> toConvert)
{
 vector<vector<int>> converted(12, vector<int>(5));
 int temp, row, col;
 for(row = 0; row < 12; row++)
        for(col =0; col<5; col++){
            temp = atoi(toConvert[row][col].c_str());
            converted[row][col] = temp;
			//cout << converted[row][col] << endl;
        }
return converted;
}

int RWModule::calcAvg(vector<vector<int>> ParsedInt)
{
	int sum = 0;
	int avg;
	int lines = 0;
	...
	{
		if (ParsedInt[count][4] == 0)
			sum = sum + ParsedInt[count][4];
		else
		{
			sum = sum + ParsedInt[count][4];
			lines++;
